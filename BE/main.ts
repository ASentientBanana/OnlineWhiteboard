...
socketIO.on("connection", (socket: any) => {
  //event sta se desava kada se konektuje na socket
  ...
  socket.on("join_room", (room_name: string) => {
    //event poziva se ovo kada se desi join_room
  ...
  socket.on("draw", (e: any) => {
    //soket koji prosledjuje koordinate crteza svima u sobi
  ...
  socket.on("chat-message", (msg: any) => {
    ...
    if (findWord(rooms[rooms[socket.id]].word, msg.body.toLowerCase())) {
      socketIO.to(rooms[socket.id]).emit("WINNER", msg.name);
    }
  });
  //socket disconnect
  ...
  socket.on("disconnect", () => {
    try {
      if (rooms[rooms[socket.id]] && rooms[rooms[socket.id]].users.length > 0)
        rooms[rooms[socket.id]].users.shift();
