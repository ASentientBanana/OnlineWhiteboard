...
export const Whiteboard = ({ socket, name }: any) => {
  ...
  useEffect(() => {
    ...
    if (canvasRef.current && whiteboardContainer.current)
      if (myCanvas === undefined)
        setMyCanvas(
          new WhiteboardBuilder(canvasRef.current)
            ...
            .build()
        ); // Ovo je poziv buildera koji ce da setuje potrebne stvari canvasu
    //   if(myCanvas === undefined)
  ...
  const socketListenerEvents = () => {
    socket.on("isOwner", (e: number) => {
      // is owner event se aktivira ako server posalje is oWner i setuje da si owner...
      if (e === 200) {
        setIsOwner(true);
      }
    });
    ...
    socket.on("draw-shape", (e: any) => {
      //salje se draw-shape event serveru
  ...
  const startPos = (e: any) => {
    // ovo se poziva kada se kliken na canvas
  ...
  const endPos = (e: any) => {
    // ovo se poziva kada se zavrsi  klik na canvas
    if (!drawing) return;
    setIsClicked(true);
    myCanvas?.clearShapeArray();
    if (shapeStartPositions) {
      ...
      const [endOffsetX, endOffsetY] = getOffset(
        e.clientX,
        e.clientY,
        myCanvas!.canvas
      );
  ...
  const localShapeHandler = (drawEvent: any) => {
    ...
    if (shapeStartPositions)
      myCanvas?.drawShape(
        shapeStartPositions[0],
        shapeStartPositions[1],
        endOffsetX,
        endOffsetY,
        { color, lineWidth, shape: drawMode, isLocal: true }
      );
  };
  ...
  const serverShapeHandler = (drawShapeEvent: any) => {
    ...
    myCanvas?.drawShape(
      drawShapeEvent.startX,
      drawShapeEvent.startY,
      drawShapeEvent.endOffsetX,
      drawShapeEvent.endOffsetY,
      options
    );
  };
  const getOffset = (
  ...
  const draw = (
    ...
    if (isLocal) {
      ...
      let data: any;
      if (drawMode === 0)
        data = myCanvas!.draw(offsetX, offsetY, {
          color,
          lineWidth,
          lineCap: "round",
        });
  ...
  const submitWord = () => {
    // salje se rec koju mora da pogodi neko
