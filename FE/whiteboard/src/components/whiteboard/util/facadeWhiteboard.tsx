...
export default class WhiteboardFacade {
  ...
  draw(
    ...
    positionY: number,
    {
      ...
      lineWidth = 50,
      lineCap = "round",
    }: whiteboardTypes.IDrawingOptions
  ) {
  ...
  setCanvasImageData() {
    // setuje trenutno  stanje canvasa u promenjivu canvasImageData
    this.canvasImageData = this.ctx?.getImageData(
      0,
      0,
      this.canvas.width,
      this.canvas.height
  ...
  drawShape(
    ...
    endPositionY: number,
    {
      ...
      shape = 1,
      isLocal = true,
    }: whiteboardTypes.IShapeDrawingOptions
  ) {
